...
struct SystemParam
{
	...
	QString nSystemGroup;		//<立即> 机器人系统当前的Group，0-主Group，1-辅助Group
...
{
	//8位参数（预留40）
		bool bEnableDynamicCoordinate;   // <复位生效> 是否使能动态坐标系跟踪。0不使能 1使能，默认0
		bool rev8[39];
		...
		unsigned short nGroupNo;				// <重启生效>	通道控制对应的主Group的索引，≥0，默认1
		//				0表示该通道没有group控制，全是外部轴
		//				1对应第一个Group（GroupConfig【0】）

		unsigned short nAuGroupNo;			// <重启生效>	通道控制对应的辅助Group的索引，≥0，默认0
		//				0表示该通道没有辅助group
		//				1对应第一个Group
		...
		//64位参数（预留100）
		double dynTransMasterOrigin[6];		//  <复位生效>  动态跟踪：master的MCS相对当前group的MCS的坐标系变换关系
		double dynTransPcsToMasterTcs[6];	//  <复位生效>  动态跟踪：PCS相对master的TCS的坐标系变换关系
